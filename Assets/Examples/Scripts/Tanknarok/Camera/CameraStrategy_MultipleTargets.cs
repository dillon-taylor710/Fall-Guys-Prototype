...
namespace FusionExamples.Tanknarok
{
	public class CameraStrategy_MultipleTargets : CameraStrategy
	{
		...
		void UpdatePositionAndRotation()
		{
			...
			_myCamera.transform.localPosition = Vector3.Lerp(_myCamera.transform.localPosition, _offset, Time.deltaTime * 10f) + _screenShaker.finalPositionalShake;
			_myCamera.transform.rotation = Quaternion.Lerp(_myCamera.transform.rotation, Quaternion.Euler(cameraTiltAngle, 0, 0), Time.deltaTime * 10f) * _screenShaker.finalRotationShake;

			//Camera parent position
			transform.position = Vector3.Lerp(transform.position, _averageTarget, Time.deltaTime * _moveSpeed);
		}
