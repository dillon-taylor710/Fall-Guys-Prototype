...
namespace Newtonsoft.Json.Utilities
{
	...
	internal static class DateTimeUtils
	{
		...
		{
			DateTime result;
			if (DateTime.TryParseExact(text, dateFormatString, culture, DateTimeStyles.RoundtripKind, out result))
			{
		...
		private static bool TryParseDateTimeOffsetExact(string text, string dateFormatString, CultureInfo culture, out DateTimeOffset dt)
		{
			...
			if (DateTimeOffset.TryParseExact(text, dateFormatString, culture, DateTimeStyles.RoundtripKind, out result))
			{
				dt = result;
				return true;
			}
		...
		private static bool TryReadOffset(StringReference offsetText, int startIndex, out TimeSpan offset)
		{
			...
			int value;
			if (ConvertUtils.Int32TryParse(offsetText.Chars, startIndex + 1, 2, out value) != ParseResult.Success)
			{
