...
namespace Newtonsoft.Json.Serialization
{
	...
	internal class JsonSerializerInternalWriter : JsonSerializerInternalBase
	{
		...
		private void SerializeValue(JsonWriter writer, object value, JsonContract valueContract, JsonProperty member, JsonContainerContract containerContract, JsonProperty containerProperty)
		{
			...
			{
				JsonConverter jsonConverter = ((member != null) ? member.Converter : null) ?? ((containerProperty != null) ? containerProperty.ItemConverter : null) ?? ((containerContract != null) ? containerContract.ItemConverter : null) ?? valueContract.Converter ?? Serializer.GetMatchingConverter(valueContract.UnderlyingType) ?? valueContract.InternalConverter;
		...
		private void SerializeObject(JsonWriter writer, object value, JsonObjectContract contract, JsonProperty member, JsonContainerContract collectionContract, JsonProperty containerProperty)
		{
			...
			for (int i = 0; i < contract.Properties.Count; i++)
			{
				...
				try
				{
					...
					object memberValue;
					if (CalculatePropertyValues(writer, value, contract, member, jsonProperty, out memberContract, out memberValue))
					{
			...
			if (!property.Ignored && property.Readable && ShouldSerialize(writer, property, value) && IsSpecified(writer, property, value))
			{
				...
				if (ShouldWriteProperty(memberValue, property))
				{
					...
					if (memberValue == null)
					{
						...
						switch (property._required ?? ((jsonObjectContract != null) ? jsonObjectContract.ItemRequired : null) ?? Required.Default)
						{
		...
		{
			bool flag = (ResolveIsReference(contract, member, containerContract, containerProperty) ?? HasFlag(Serializer._preserveReferencesHandling, PreserveReferencesHandling.Arrays)) && (member == null || member.Writable);
		...
		private void SerializeDictionary(JsonWriter writer, IDictionary values, JsonDictionaryContract contract, JsonProperty member, JsonContainerContract collectionContract, JsonProperty containerProperty)
		{
			...
			{
				IDisposable obj2 = enumerator as IDisposable;
				if (obj2 != null)
				{
					obj2.Dispose();
				}
		...
		private string GetPropertyName(JsonWriter writer, object name, JsonContract contract, out bool escape)
		{
			...
			string s;
			if (TryConvertToString(name, name.GetType(), out s))
			{
				escape = true;
				return s;
			}
